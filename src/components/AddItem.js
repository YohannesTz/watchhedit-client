...
const AddItem = ({itemType, isOpen, onClose }) => {
  ...
  switch (itemType) {
    ...
    case itemTypes.ANIME:
      break;
    case itemTypes.BOOK:
      return <AddBook isOpen={isOpen} onClose={onClose}/>
...
const AddBook = (isOpen, onClose) => {
  ...
  const { createItemLoading, createItemError, createItemSuccess } = useSelector(
    (state) => state.book
  );
